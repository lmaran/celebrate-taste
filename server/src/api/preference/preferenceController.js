...
exports.update = function(req, res) {
    ...
    preferenceValidator.all(req, res, function(errors) {
        if (errors) {
            ...
            if (preference.option1) preference.option1 = preference.option1.toUpperCase();
            if (preference.option2) preference.option2 = preference.option2.toUpperCase();
...
exports.getNextDates = function(req, res) {
    ...
    preferenceService.getNextDates(todayStr, function(err, dates) {
        ...
        var result = [];
        if (dates.length > 0) result = dates[0].nextDates;
        res.json(result);
    });
};
